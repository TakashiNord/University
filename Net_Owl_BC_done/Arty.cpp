...
class TList // defines classes used internally by Arty, such as PTList
{
public:
         TList( int Max );
         void Redraw( HDC );
...
private: int  MaxLines,     X1, Y1, X2, Y2,     MaxDelta,     ColorDuration,
              IncrementCount,           DeltaX1, DeltaY1, DeltaX2, DeltaY2,      CurrentLine;
         long PenColor;
};
...
class TArtyWindow : public TWindow
{
private:
         PTList List,    BigLineList,   IconicLineList;
         int  TextHeight;
         BOOL Iconized;     BOOL Paused;
//       PTStatic StaticControl;
public:
         TArtyWindow( PTWindowsObject AParent);
         virtual ~TArtyWindow();
...
TList::TList( int Max )
{
  MaxLines = ( Max<MaxLineCount )?Max:MaxLineCount;
  CurrentLine = 1;
  Xmax = 0;
  Ymax = 0;
...
{
  int TestX = X + DeltaX;
  if ((TestX < 1) || (TestX > Xmax)) { TestX = X;   DeltaX = -DeltaX; };
  X = TestX;
};
...
{
  int TestY = Y + DeltaY;
  if ((TestY < 1) || (TestY > Ymax)) {  TestY = Y;  DeltaY = -DeltaY;  };
  Y = TestY;
};
...
void TList::ScaleTo( int NewXmax, int NewYmax )
{
  ...
  if ((!Xmax) || (!Ymax)) { /* at startup, Xmax and Ymax are zero */
    Xmax = NewXmax;  Ymax = NewYmax;  ResetLines();
  } else {
    RatioX = NewXmax / Xmax;    RatioY = NewYmax / Ymax;
    X1 = X1 * RatioX;  X2 = X2 * RatioX;
    Y1 = Y1 * RatioY; Y2 = Y2 * RatioY;
    for( I = 0; I < MaxLines; I++) {
...
{
  Draw(DC, Line[Index].LX1,Line[Index].LY1,
           Line[Index].LX2,Line[Index].LY2,      Line[Index].Color);
};
...
{
  Draw(DC, Line[Index].LX1,Line[Index].LY1,
           Line[Index].LX2, Line[Index].LY2,       RGB(0,0,0));
};

/* Redraw all the lines in the array. */
void TList::Redraw( HDC DC )
{
  for( int I = 0; I < MaxLines; I++ )  DrawLine(DC, I);
};
...
void TQuadList::DrawLine( HDC DC, int Index )
{
  Draw(DC, Line[Index].LX1,Line[Index].LY1,
           Line[Index].LX2,Line[Index].LY2,         Line[Index].Color);
  Draw(DC, Xmax - Line[Index].LX1,Line[Index].LY1,
           Xmax - Line[Index].LX2,Line[Index].LY2,     Line[Index].Color);
  Draw(DC, Line[Index].LX1,Ymax - Line[Index].LY1,
           Line[Index].LX2, Ymax - Line[Index].LY2,     Line[Index].Color);
  Draw(DC, Xmax - Line[Index].LX1,Ymax - Line[Index].LY1,
           Xmax - Line[Index].LX2,Ymax - Line[Index].LY2,      Line[Index].Color);
};
...
void TQuadList::EraseLine( HDC DC, int Index )
{
  Draw(DC, Line[Index].LX1,Line[Index].LY1,
           Line[Index].LX2,Line[Index].LY2,       RGB(0,0,0));
  Draw(DC, Xmax - Line[Index].LX1,Line[Index].LY1,
           Xmax - Line[Index].LX2,Line[Index].LY2,        RGB(0,0,0));
  Draw(DC, Line[Index].LX1,Ymax - Line[Index].LY1,
           Line[Index].LX2, Ymax - Line[Index].LY2,        RGB(0,0,0));
  Draw(DC, Xmax - Line[Index].LX1,Ymax - Line[Index].LY1,
           Xmax - Line[Index].LX2,Ymax - Line[Index].LY2,       RGB(0,0,0));
};

//----------- TArtyWindow ------------------------------------------

TArtyWindow::TArtyWindow( PTWindowsObject AParent) :
...
//    "Press Left Button to pause, Right Button to Clear",10,10,10,10,0);
 ...
 Attr.X = 200;    Attr.Y = 10;     Attr.W = 50;      Attr.H = 50;
  Iconized = False;  TextHeight = 20;  Paused = FALSE;
...
{  delete BigLineList;  delete IconicLineList; };

void TArtyWindow::GetWindowClass( WNDCLASS& aWndClass )
{
...
void TArtyWindow::WMSize( TMessage& Message )
{
  ...
  NewXmax = Message.LP.Lo;  NewYmax = Message.LP.Hi;
  if (IsIconic(HWindow)) {
    if (!Iconized) {  Iconized = TRUE;    List = IconicLineList;  }
  } else {
    if (Iconized) {   Iconized = FALSE;   List = BigLineList;   };
...
void TArtyWindow::TimerTick()
{
  ...
  if (!Paused) { DC = GetDC(HWindow);  List->LineTick(DC);
