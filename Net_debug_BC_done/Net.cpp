...
int  fl,fl1,fl2,fl3;/*Флажки для циклов*/
 ...
 float Pr; FILE *rf,*hr;
/**************************************************************/
int NetCalc(char *nameLength_R,char *namePrice_C,char *nameIntense_D,
             char *nameOut, int n,
...
R = (float far *) farmalloc(n*n*sizeof(float));/*м триц  пропускных способностей*/
C = (float far *) farmalloc(n*n*sizeof(float));/*м триц  стоимости*/
X = (float far *) farmalloc(n*n*sizeof(float));/*м триц  оптим льного поток }*/
float far *D;float far *nQ; //float  D[n],nQ[n];
D = (float far *) farmalloc(n*sizeof(float)); /*м триц  интенсивностей*/
nQ = (float far *) farmalloc(n*sizeof(float));  /*м триц  количеств */
int far *nN;int far*Spot;
nN = (int far *) farmalloc(n*sizeof(int));/*м триц  пометок вершин*/
Spot= (int far *) farmalloc(n*sizeof(int));/*м триц  индексов р ссмотренных вершин*/
        ...
        switch (FlagCase){
          ...
          case 3:/*Выбор вершины сл. образом*/
	         ...
	         while (fl1==0)
	           ...
	           if (Spot[j]==0)
                    if ((nN[j]!=0) || (nQ[j]!=0))
                       { fl1=1;i=j;Spot[j]=1;l++; } else { ; }  else { ; }
                  }
	         break;
          default:break;
                }
...
/* free the memory */ farfree(R);farfree(nN);farfree(nQ);farfree(Spot);
/* Вывод результатов */
SumStr=0.0;
if (FlagCalc!=0) 
{ if((hr = fopen(namePrice_C, "r"))==NULL) return(-1); /*Формирование матрицы С[i,j] из файла */
  for(i=0;i<n;i++) 
  { for(j=0;j<n;j++){ fscanf(hr,"%f",&Pr);SumStr+=(Pr*X[i*n+j]); /* fprintf(rf," %8.3g ",Pr);*/ }
   }
  fclose(hr);
}

fprintf(rf,"\nРасстановкаПометок (%d) , ВыборВершины(%d)\n",FlagSpot,FlagCase);
if (SumPr==0.0000000000) { fprintf(rf,"Поток ОПТИМАЛЬНЫЙ , т.к. D[i]=0, i=1..%d",n); }
...
/* free the memory */  farfree(X);farfree(C);farfree(D);
fclose(rf); 
return (0);
}
...
int main()
{ int i;
i=NetCalc("c:\\programs\\bc\\varl\\net\\length3.prn",
          "c:\\programs\\bc\\varl\\net\\price3.prn",
          "c:\\programs\\bc\\varl\\net\\intense3.prn",
          "c:\\programs\\bc\\out\\__out3.prn",
           12, 1,2 ,1 ,1, 500);
printf("%d",i);
return(0);
}